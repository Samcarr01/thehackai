...
export default function AuthenticatedMobileNav({ userEmail, isAdmin }: AuthenticatedMobileNavProps) {
  const [isOpen, setIsOpen] = useState(false)

  const handleSignOut = async () => {
    try {
      // Import auth dynamically to avoid import issues
        ...
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 p-6">
            <div className="space-y-2">
              ...
              <Link href="/gpts" onClick={closeMenu}
                className="flex items-center px-3 py-3 text-gray-300 hover:text-white hover:bg-gray-700 rounded-lg transition-all duration-200">
                <span className="mr-3">🤖</span>
                GPTs
              </Link>
              ...
              <Link href="/settings" onClick={closeMenu}
                className="flex items-center px-3 py-3 text-white bg-purple-600 hover:bg-purple-700 rounded-lg shadow-lg transition-all duration-200">
          ...
          <div className="border-t border-gray-700 p-6">
            <button
              ...
              className="w-full flex items-center justify-center px-3 py-3 text-red-400 hover:text-red-300 hover:bg-red-900/20 rounded-lg transition-all duration-200"
            >
              <span className="mr-3">🚪</span>
              Sign Out
            </button>
